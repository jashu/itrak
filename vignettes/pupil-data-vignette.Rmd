...
The sample `pupil_data` included with the `itrak` package contains 9 experimental trials + 3 drift checks collected from an EyeLink 1000, sampling pupil area from a single eye at a frequency of 500 Hz. Each experimental trial includes a 2-second baseline period, followed by the presentation of an affective image for 6 seconds. `pupil_data` contains the following columns:
...
Inspect plots of the algorithm's performance and manually flag artifacts or remove trials as needed. 
...
In the above plot, the red bands indicate regions of the time series that have been identified as signal artifacts. The blue bands indicate regions of the time series that exceed the limits placed on relative change from baseline. In this case, this region reflects more than a 75% decrease in pupil area from the start of the trial, which exceeds the amount of change that a psychological stimulus can plausibly evoke.

# `fix_artifacts`

Remove the artifacts using `fix_artifacts` and store the results in a new variable called `pupil_corrected`. 
...
Smooth the `pupil_normed` time series using a 4 Hz low-pass filter.
...
Plot comparison of pre-smoothed vs. post-smoothed. To visualize what this is doing to the data, you need to zoom in on a single time series. (Fitting all of the trials into one point makes the curves seem smoother to the eye than they actually are.)
